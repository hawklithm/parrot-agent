...
impl UserDirectoryService for UserDirectoryServiceImpl {
    async fn list_company_users(
        ...
        let total: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM company_memberships m JOIN auth_users u ON u.id=m.principal_id WHERE m.company_id=$1 AND m.principal_type='user' AND m.status='active' AND ($2='' OR u.email ILIKE $3 OR COALESCE(u.name,'') ILIKE $3)").bind(company_id).bind(&query.query).bind(&pattern).fetch_one(pool).await?;
        let rows = sqlx::query("SELECT m.principal_id, m.status::text, u.email, u.name, u.avatar_url FROM company_memberships m JOIN auth_users u ON u.id=m.principal_id WHERE m.company_id=$1 AND m.principal_type='user' AND m.status='active' AND ($2='' OR u.email ILIKE $3 OR COALESCE(u.name,'') ILIKE $3) ORDER BY COALESCE(u.name,u.email), u.id LIMIT $4 OFFSET $5").bind(company_id).bind(&query.query).bind(&pattern).bind(query.limit as i64).bind(query.offset as i64).fetch_all(pool).await?;
    ...
    async fn list_admin_users(
        ...
        let rows = sqlx::query("SELECT u.id,u.email,u.name,u.avatar_url, EXISTS(SELECT 1 FROM instance_user_roles r WHERE r.user_id=u.id AND r.role='instance_admin') AS is_instance_admin, (SELECT COUNT(*) FROM company_memberships m WHERE m.principal_id=u.id AND m.principal_type='user' AND m.status='active')::int AS membership_count FROM auth_users u WHERE ($1='' OR u.email ILIKE $2 OR COALESCE(u.name,'') ILIKE $2) ORDER BY COALESCE(u.name,u.email),u.id LIMIT $3 OFFSET $4").bind(&query.query).bind(&pattern).bind(query.limit as i64).bind(query.offset as i64).fetch_all(pool).await?;
