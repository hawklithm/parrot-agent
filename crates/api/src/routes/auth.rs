...
async fn create_session_response(state: &AppState, user: AuthUser) -> Result<Response, AuthError> {
    ...
    let session = PgAuthSessionRepository::new(state.pool.clone())
        .create(session)
        .await
...
/// - Board 用户：id / userId / companyId / role / isInstanceAdmin / source。
    ...
    match actor {
        AuthorizationActor::Board {
            ...
            let role = memberships
                ...
                .unwrap_or("member");
            json!({
                // A browser session overwrites this stable fallback with the
                // persisted auth_sessions id in get_session.  API-key and
                // local-trusted actors do not have a session row.
...
fn user_profile_value(user: &AuthUser) -> serde_json::Value {
    json!({
...
/// Load the persisted session id for a browser cookie when one is present.
/// Board API keys and local-trusted requests do not have an auth_sessions row,
/// so build_session_view keeps a stable actor-id fallback for those identities.
async fn session_id_from_headers(
    pool: &sqlx::PgPool,
    headers: &HeaderMap,
) -> Result<Option<Uuid>, AuthError> {
    let cookie_name = format!(
        "{}-session",
        auth_cookie_prefix(&std::env::var("INSTANCE_ID").unwrap_or_else(|_| "default".to_string()))
    );
    let token = headers
        .get(header::COOKIE)
    ...
    .fetch_optional(pool)
    .await
    .map_err(|error| AuthError::internal(format!("Failed to load auth session: {}", error)))
}
...
async fn get_session(
    ...
    let mut session = build_session_view(&actor);
    if actor.is_board() {
        if let Some(session_id) = session_id_from_headers(&state.pool, &headers).await? {
            if let Some(session_object) = session.as_object_mut() {
                session_object.insert("id".to_string(), json!(session_id));
            }
        }
    }
    let user = match &actor {
        AuthorizationActor::Board { user_id, .. } => {
            ...
            loaded.map(|u| user_profile_value(&u))
        }
        _ => None,
    };
...
async fn get_profile(
    ...
    Ok(Json(user_profile_value(&user)))
}
...
pub struct ProfileUpdate {
    #[serde(default)]
    pub name: Option<String>,
    /// `Some(None)` explicitly removes the image; `None` leaves it unchanged.
    /// The aliases keep older clients writable during rollout.
...
async fn update_profile(
    ...
    Ok(Json(user_profile_value(&updated)))
}
...
async fn demote_instance_admin(
    ...
    .rows_affected();
    if deleted == 0 {
        return Err(AuthError::not_found("Instance admin role not found"));
    }
    Ok(Json(json!({
        "userId": user_id,
...
async fn get_user_company_access(
    ...
    assert_instance_admin(&actor)?;
    Ok(Json(user_company_access_response(&state.pool, user_id).await?).into_response())
}

fn api_membership_status(status: &str) -> &'static str {
    match status {
        "pending" => "pending",
        "active" => "active",
        "suspended" => "suspended",
        // The legacy Parrot schema called the soft-deleted state `inactive`;
        // Paperclip exposes the same state as `archived`.
        "archived" | "inactive" => "archived",
        _ => "archived",
    }
}

async fn user_company_access_response(
    pool: &sqlx::PgPool,
    user_id: Uuid,
) -> Result<serde_json::Value, AuthError> {
    use sqlx::Row;

    let user = PgAuthUserRepository::new(pool.clone())
        .find_by_id(user_id)
        .await
        .map_err(|error| AuthError::internal(error.to_string()))?
        .ok_or_else(|| AuthError::not_found("User not found"))?;
    let is_instance_admin = sqlx::query_scalar::<_, bool>(
        "SELECT EXISTS(
    ...
    let rows = sqlx::query(
        "SELECT cm.id, cm.company_id, c.name, c.status::text AS company_status,
    ...
    let company_access = rows
        ...
        .map(|row| {
            json!({
                ...
                "principalType": "user",
                "principalId": user_id,
                "status": api_membership_status(row.get::<String, _>("status").as_str()),
                "membershipRole": row.get::<String, _>("role"),
                "createdAt": row.get::<chrono::DateTime<chrono::Utc>, _>("created_at"),
                "updatedAt": row.get::<chrono::DateTime<chrono::Utc>, _>("updated_at"),
                "companyName": row.get::<String, _>("name"),
                "companyStatus": row.get::<String, _>("company_status"),
            })
        })
        .collect::<Vec<_>>();
...
#[serde(rename_all = "camelCase")]
struct UpdateUserCompanyAccessPayload {
    #[serde(default)]
    company_ids: Vec<Uuid>,
    // Kept as a compatibility bridge for older clients that sent one company
    // at a time. The canonical Paperclip contract is companyIds.
...
async fn update_user_company_access(
    ...
    Extension(actor): Extension<AuthorizationActor>,
    Path(user_id): Path<Uuid>,
    Json(payload): Json<UpdateUserCompanyAccessPayload>,
) -> Result<Response, AuthError> {
    assert_instance_admin(&actor)?;
    ...
    let mut selected_company_ids = payload.company_ids;
    if selected_company_ids.is_empty() {
        if let Some(company_id) = payload.company_id {
            selected_company_ids.push(company_id);
        }
    }
    selected_company_ids.sort_unstable();
    selected_company_ids.dedup();
    ...
    if let Some(status) = payload.status.as_deref() {
        if !matches!(status, "active" | "inactive" | "archived") {
            return Err(AuthError::bad_request("status must be active, inactive, or archived"));
        }
    }
    ...
    let existing = sqlx::query(
        "SELECT company_id, membership_role::text AS role
    ...
    let existing_ids: std::collections::HashSet<Uuid> = existing
        .iter()
        .map(|row| row.get::<Uuid, _>("company_id"))
        .collect();
    let selected_ids: std::collections::HashSet<Uuid> = selected_company_ids.iter().copied().collect();

    sqlx::query(
        "UPDATE company_memberships
         SET status = CASE
                 WHEN company_id = ANY($1) THEN 'active'::company_membership_status
                 ELSE 'archived'::company_membership_status
             END,
             updated_at = NOW()
         WHERE principal_type = 'user'::principal_type AND principal_id = $2",
    )
    ...
    for company_id in selected_ids.difference(&existing_ids) {
        sqlx::query(
            "INSERT INTO company_memberships
                 (company_id, principal_type, principal_id, membership_role, status)
             VALUES ($1, 'user'::principal_type, $2, 'operator'::membership_role,
                     'active'::company_membership_status)",
        )
        .bind(company_id)
        .bind(user_id)
        .execute(&mut *tx)
...
async fn claim_join_request_api_key(
    ...
    // Paperclip 在这条路径上把 responsible user 记为
    // `approvedByUserId ?? requestingUserId`
    // （`server/src/routes/access.ts:4426`），即「批准人优先，否则申请人」。
    // Parrot 的对应列是 `reviewed_by_user_id` / `requester_user_id`。
    let responsible_user_id = row
        .get::<Option<Uuid>, _>("reviewed_by_user_id")
        .or_else(|| row.get::<Option<Uuid>, _>("requester_user_id"));
    sqlx::query(
        "INSERT INTO agent_api_keys
             (id, agent_id, company_id, name, key_hash, scope, responsible_user_id, created_at, updated_at)
         VALUES ($1, $2, $3, 'initial-join-key', $4, $5, $6, $7, $7)",
    )
    .bind(key_id)
    .bind(agent_id)
    .bind(company_id)
