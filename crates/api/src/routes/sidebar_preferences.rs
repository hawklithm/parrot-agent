...
//!
//! - `GET/PUT /sidebar-preferences/me`：当前用户对公司列表的顺序偏好（user 级）。
//! - `GET/PUT /companies/:company_id/sidebar-preferences/me`：公司在侧栏的
//!   project 顺序偏好（company 级，PUT 写 `sidebar_preferences.project_order_updated` 审计）。
...
async fn get_user_sidebar_preferences(
    ...
    let ordered = row
        ...
        .unwrap_or_default();
    Ok(Json(ordered_ids_json(&ordered.iter().filter_map(|v| v.as_str().map(String::from)).collect::<Vec<_>>())))
}
...
/// GET /companies/:company_id/sidebar-preferences/me
...
/// PUT /companies/:company_id/sidebar-preferences/me
